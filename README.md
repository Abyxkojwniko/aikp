...
- 模组中的“条件触发 -> 技能检定 -> 成功/失败感知 -> 后续技能或 SAN 检定”会重建为可执行事件链；未通过的分支不会进入叙事上下文。
- 条件感知按调查员分别记入 `knowledge_facts` 与不可变知识事件日志；当前调查员只能使用自己的观察和公开事实，不会自动继承其他调查员看到的幻象或线索。
- 同一 `current_scene` 可按调查员属性、知识或检定结果投影为不同 `perception_layers`：场景名称、完整描述、可见对象及对象外观分别计算，未激活的视图不会进入叙事提示。
- `perception_only` 对象只允许检查或阅读，不能拿取、破坏、开锁或写入物理世界事件；感知到的真实隐藏线索也只对该调查员可见，不会自动写入全队发现记录。
